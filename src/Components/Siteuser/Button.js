...
export default class SwipeableButton extends Component {
  ...
  componentDidMount() {
    ...
    this.containerWidth = container.current.clientWidth - 40;
  }
  ...
  updateSliderStyle =()=> {
    if(this.unmounted || this.state.unlocked) return;
    slider.current.style.left = (this.sliderLeft + 40)+'px';
  }
  ...
  getText =()=> {
    return this.state.unlocked ? (this.props.text_unlocked || 'CONFIRMED') : (this.props.text || 'SLIDE')
  }
